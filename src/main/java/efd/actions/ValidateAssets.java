	...
	public void execute() throws Exception {
		...
		for (i = 0; i < als.size(); i++) {
			...
			if (!(als.get(i).getNumberOwnedAtStart() >= 0)) {
				...
				als.get(i).setStatus(Status.Valid);
			}
		...
		for (i = 0; i < at.size(); i++) {
			...
			if (at.get(i).getNumberOwned() < 0) {
				at.get(i).setStatus(Status.Invalid);
				tradeIsInvalid = true;
				...
				at.get(i).setStatus(Status.Valid);
		...
		for (i = 0; i < afs.size(); i++) {
			if (!(afs.get(i).getQuantity() >= 0)) {
				afs.get(i).setStatus(Status.Invalid);
				fsIsInvalid = true;
				...
				afs.get(i).setStatus(Status.Valid);
			}
		...
		for (i = 0; i < atree.size(); i++) {
			...
			if (!(atree.get(i).getNumberOwned() >= 0)) {
				atree.get(i).setStatus(Status.Invalid);
				treeIsInvalid = true;
				...
				atree.get(i).setStatus(Status.Valid);
			}
		...
		for (i = 0; i < ac.size(); i++) {
			...
			if (ac.get(i).getAmount().intValue() > 0 && ac.get(i).getExchangeRate().intValue() > 0) {
				ac.get(i).setStatus(Status.Valid);

			} else if (ac.get(i).getResourceSubType() == null) {
				ac.get(i).setStatus(Status.Invalid);
				cashIsInvalid = true;
		...
		for (i = 0; i < crop.size(); i++) {
			...
			if (crop.get(i).getUnitsSold() > 0 && crop.get(i).getPricePerUnit() <= 0) {
				...
				crop.get(i).setStatus(Status.Invalid);
				cropIsInvalid = true;
			...
			// }
				...
				crop.get(i).setStatus(Status.Valid);
		...
		for (i = 0; i < lss.size(); i++) {
			...
			if (lss.get(i).getUnitsSold() > 0 && lss.get(i).getPricePerUnit() <= 0) {
				lss.get(i).setStatus(Status.Invalid);
				...
				lss.get(i).setStatus(Status.Valid);
		...
		for (i = 0; i < lsp.size(); i++) {
			...
			if (lsp.get(i).getUnitsSold() > 0 && lsp.get(i).getPricePerUnit() <= 0) {
				...
				lsp.get(i).setStatus(Status.Invalid);
				lspIsInvalid = true;
				...
				lsp.get(i).setStatus(Status.Valid);
		...
		for (i = 0; i < emp.size(); i++) {
			...
			if ((emp.get(i).getPeopleCount() < 0) || (emp.get(i).getUnitsWorked() < 0)
				...
				empIsInvalid = true;
				emp.get(i).setStatus(Status.Invalid);
				...
				emp.get(i).setStatus(Status.Valid);
			}
		...
		for (i = 0; i < tran.size(); i++) {
			...
			if (tran.get(i).getUnitsSold() > 0 && tran.get(i).getPricePerUnit() <= 0) {
				...
				tran.get(i).setStatus(Status.Valid);
			}
		...
		for (i = 0; i < wf.size(); i++) {
			...
			if (wf.get(i).getUnitsSold() > 0 && wf.get(i).getPricePerUnit() <= 0) {
				wf.get(i).setStatus(Status.Invalid);
				wfIsInvalid = true;
				addWarning(
				...
				wf.get(i).setStatus(Status.Valid);
			}
		...
		if (isHH) {
			for (i = 0; i < ins.size(); i++) {
				...
				if (ins.get(i).getUnitsPurchased() > 0 && ins.get(i).getPricePerUnit() <= 0) {
					ins.get(i).setStatus(Status.Invalid);
					inpIsInvalid = true;
					...
					ins.get(i).setStatus(Status.Valid);
