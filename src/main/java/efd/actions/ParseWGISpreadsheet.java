...
public class ParseWGISpreadsheet extends CollectionBaseAction implements IForwardAction, JxlsConstants, IFilePersistor {
	...
	public void execute() throws Exception {
		...
		if (wgi.getSpreadsheet().isEmpty()) {
			...
			return;
		} else if ((wgi.getStatus().equals(Status.PartParsed)
				|| wgi.getStatus().equals(Status.FullyParsed))) {
			addWarning("Cannot Parse Interview Spreadsheet - Already Parsed");
			return;
		} else if (wgi.getStatus().equals(Status.Validated)) {
				...
				case ASSETLIVESTOCK:
					try {
						...
						if ((rst = checkSubType(cell[i][j][0].getStringCellValue(), // is this a valid resource type?
							...
							als.setStatus(Asset.Status.Valid);
							if (!checkSubTypeEntered(als.getUnit(), rst))
								als.setStatus(Asset.Status.Invalid);
				...
				case ASSETTRADEABLE:
					try {
						...
						if ((rst = checkSubType(cell[i][j][0].getStringCellValue(), // is this a valid resource type?
							...
							atrade.setStatus(Asset.Status.Valid);
							if (!checkSubTypeEntered(atrade.getUnit(), rst))
								atrade.setStatus(Asset.Status.Invalid);
				...
				case ASSETTREE:
					try {
						...
						if ((rst = checkSubType(cell[i][j][0].getStringCellValue(), // is this a valid resource type?
							...
							atree.setStatus(Asset.Status.Valid);
							if (!checkSubTypeEntered(atree.getUnit(), rst))
								atree.setStatus(Asset.Status.Invalid);
					...
					try {
						...
						for (icurr = 0; icurr < currency.size(); icurr++) {
							if (currency.get(icurr).getCurrency().equals(cell[i][j][0].getStringCellValue())) {
								...
								acash.setStatus(Asset.Status.Valid);
								acash.setResourceSubType(rst);
								break;
							} else {
								acash.setStatus(Asset.Status.Invalid);
							}
				...
				case CROPS:
					...
					try {
						...
						if ((rst = checkSubType(cell[i][j][0].getStringCellValue(), // is this a valid resource type?
							...
							acrop.setStatus(Asset.Status.Valid);
							if (!checkSubTypeEntered(acrop.getUnit(), rst))
								acrop.setStatus(Asset.Status.Invalid);
				...
				case LIVESTOCKSALE:
					...
					try {
						...
						if ((rst = checkSubType(cell[i][j][0].getStringCellValue(), // is this a valid resource type?
							...
							alss.setStatus(Asset.Status.Valid);
							if (!checkSubTypeEntered(alss.getUnit(), rst))
								alss.setStatus(Asset.Status.Invalid);
				...
				case LIVESTOCKPRODUCT:
					try {
						...
						if ((rst = checkSubType(alsp.getLivestockType() + " " + alsp.getLivestockProduct(), // is this a
							...
							alsp.setStatus(Asset.Status.Valid);
							isCorrectRST = true;
							if (!checkSubTypeEntered(alsp.getUnit(), rst))
								alsp.setStatus(Asset.Status.Invalid);
				...
				case EMPLOYMENT:
					try {
						...
						if ((rst = checkSubType(cell[i][j][0].getStringCellValue(), // is this a valid resource type?
							...
							aemp.setStatus(Asset.Status.Valid);
							if (!checkSubTypeEntered(aemp.getUnit(), rst))
								aemp.setStatus(Asset.Status.Invalid);
				...
				case TRANSFER:
					try {
						...
						if ((rst = checkSubType(cell[i][j][2].getStringCellValue(), // is this a valid resource type -
							...
							at.setStatus(Asset.Status.Valid);
							if (!checkSubTypeEntered(at.getUnit(), rst))
								at.setStatus(Asset.Status.Invalid);
				...
				case WILDFOOD:
					try {
						...
						if ((rst = checkSubType(cell[i][j][0].getStringCellValue(), // is this a valid resource type?
							...
							awf.setStatus(Asset.Status.Valid);
							isCorrectRST = true;
							if (!checkSubTypeEntered(awf.getUnit(), rst)) {
								em("checksubtypeentered fail ");
								awf.setStatus(Asset.Status.Invalid);
							}
						} else {
				...
				case FOODPURCHASE:
					try {
						...
						if ((rst = checkSubType(cell[i][j][0].getStringCellValue(), // Food Purchase
							...
							afp.setStatus(Asset.Status.Valid);
						 ...
						 */
							...
							if (!checkSubTypeEntered(afp.getUnit(), rst))
								afp.setStatus(Asset.Status.Invalid);
				...
				case NONFOODPURCHASE:
					...
					try {
						...
						if ((rst = checkSubType(cell[i][j][0].getStringCellValue(), // is this a valid resource type?
							...
							anfp.setStatus(Asset.Status.Valid);
							if (!checkSubTypeEntered(anfp.getUnit(), rst))
								anfp.setStatus(Asset.Status.Invalid);
