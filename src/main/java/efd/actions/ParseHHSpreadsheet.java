...
public class ParseHHSpreadsheet extends CollectionBaseAction
	...
	public void execute() throws Exception {
		...
		System.out.println("study reference year" + hhi.getStudy().getReferenceYear());

		if (hhi.getStatus() == null) {
			em("status is empty"); // do nothing and continue parse
		}

		else if ((hhi.getStatus().equals(Status.PartParsed)
				|| hhi.getStatus().equals(Status.FullyParsed))) {
			addWarning("Cannot Parse Interview Spreadsheet - Already Parsed");
			nextAction = null;
			return;
		} else if (hhi.getStatus().equals(Status.Validated)) {
		...
		try {
			...
			if (schema != null) {
				...
				ps = con.prepareStatement(
						"select id,name,data from " + schema + "OXFILES where ID = '" + spreadsheetId + "'");
			} else
				ps = con.prepareStatement("select id,name,data from OXFILES where ID = '" + spreadsheetId + "'");

			ResultSet rs = ps.executeQuery();

			rs.next();

			if(rs.getString(2).contains("xlsx"))
			{
				addError("Only Excel .xls files are supported for parsing");
				nextAction = null;
				return;
			}
	...
	/*
	 * get Interview details and validate if this is correct study
	 */
	...
	private Boolean getInterviewDetails(Workbook wb, Household hhi) {
		...
		if (!icell.getStringCellValue().trim().equals(studyName.trim())) // Not for this project
		{
			...
			addError("Questions will be inconsistent for Study / Project ");
		...
		if (hhName.isEmpty()) {
			...
			/*
			 * If Household Name has format of "123 - Nameof Household" then assume the NNN
			 * - is the Household Number # 462
			 */
			System.out.println("str index - " + StringUtils.indexOf(hhName, "-"));
			System.out.println("number = "+hhi.getHouseholdNumber());
			if (hhName.indexOf("-") <= 4 && hhName.indexOf("-") > 1) 
				// HHName looks like "123 - HouseName" and "-" is near beginning
			{
				hhNumberStr = StringUtils.substringBefore(hhName, "-");
				hhNumberStr = StringUtils.chop(hhNumberStr);
				final int hhNumber = Integer.parseInt(hhNumberStr);
				
				// Has hhNumber been used before?
				
				long hhNumberCount = 0;
				hhNumberCount = hhi.getStudy().getHousehold().stream()
				.filter(p->p.getHouseholdNumber() == hhNumber).count();
				
				if(hhNumberCount>0 && hhi.getHouseholdNumber() != hhNumber )
				{
					// Number already used and saved in Study and not this number
					addError("Household Number "+hhNumber+" encoded in Spreadsheet Household Name already exists");
					nextAction = null;
					return (false);
				}
				
				
				System.out.println("Parsed HHNumber = " + hhNumber);
				hhi.setHouseholdNumber(hhNumber);
			}
		}
			...
			if (nextq.getConfigQuestion().getLevel().toString() == "Household") {
				...
				for (k = 0; k < qand.size(); k++) {
					if (qand.get(k).getQuestion().equals(nextq.getConfigQuestion().getPrompt())) {
						...
						if (nextq.getConfigQuestion().getAnswerType().equals(AnswerType.LOV)) {
							String lovType2 = nextq.getConfigQuestion().getQuestionLOVType().getId();

							List<QuestionLOV> qLOV = XPersistence.getManager()
									.createQuery("from QuestionLOV where questionLOVType_id = :lovType2")
									.setParameter("lovType2", lovType2).getResultList();

							/* check if value from spreadsheet is a valid LOV */

							for (QuestionLOV questionLOV : qLOV) {

								if (questionLOV.getLovValue().equals(qand.get(k).getAnswer())) {

									qanswer.setAnswerType(AnswerType.LOV);
									qanswer.setLovAnswer(questionLOV);
								}
	...
	private int getHHMDetails(Workbook wb, Household hhi) { // return 0 if success
		...
		int qsize = hhi.getStudy().getConfigQuestionUse().size(); 
		...
		int thisYear = hhi.getStudy().getReferenceYear();

		System.out.println("this year in hhm details = " + thisYear);
		...
		for (int hhmcol = 2; hhmcol < 22; hhmcol++) {
			...
			try {
				...
				// head = sheet.getRow(7).getCell(hhmcol,
				...
				absent = sheet.getRow(8).getCell(hhmcol, Row.MissingCellPolicy.CREATE_NULL_AS_BLANK)
						.getStringCellValue();
				...
				hhm.setHouseholdMemberNumber("HHM" + hhmNumber++); // Increment HH member number
				...
				case TRANSFER:
					try {
						...
						if (upperCaseTransferType.equals("Cash"))
							at.setTransferType(Transfer.TransferType.Cash);
						else if (upperCaseTransferType.equals("Other"))
							at.setTransferType(Transfer.TransferType.Other);
						if (upperCaseTransferType.equals("Food"))
							at.setTransferType(Transfer.TransferType.Food);
						l++;
