...
public class ModellingReports extends TabBaseAction implements IForwardAction, JxlsConstants {
	...
	private static ModellingScenario modellingScenario = null;
		...
		if (selectedOnes.length == 0) {
			...
			for (int i = 0; i < selectedOnes.length; i++) {
				...
				if (!isCommunityHasExpandabilityRule && isCopingStrategy) {
					...
					System.out.println("iscomping "+isCopingStrategy);
					System.out.println("iscommexprule "+isCommunityHasExpandabilityRule);
	...
	private void calculateDI() {
		...
		if (isOHEA) {
			...
			for (HH hh2 : uniqueHousehold) {
				...
				isChangeScenario = false;
				hh2.setHhDI(householdDI(hh2.getHousehold(), isChangeScenario, defaultDietItems));
				isChangeScenario = true;
				hh2.setHhDIAfterChangeScenario(householdDI(hh2.getHousehold(), isChangeScenario, defaultDietItems));
	...
	static Double householdDI(Household household, Boolean isChangeScenario, List<DefaultDietItem> defaultDietItems) {
		...
		try {
			if (household.getCrop().size() > 0) {
				...
				cropTI = calcCropIncome(household.getCrop(), CASH, isChangeScenario, false, false);
				cropOP = calcCropIncome(household.getCrop(), FOOD, isChangeScenario, false, false);
			}
			...
			if (household.getLivestockSales().size() > 0) {
				...
				lssTI = calcLSS(household.getLivestockSales(), CASH, isChangeScenario);
				// No output from LSS
	...
	public static Double wealthgroupInterviewDI(WealthGroupInterview wealthGroupInterview, Boolean isChangeScenario) {
		...
		try {
			...
			if (!wealthGroupInterview.getLivestockSales().isEmpty()) {
				...
				lssTI = calcLSS(wealthGroupInterview.getLivestockSales(), CASH, isChangeScenario);
				// No output from LSS
		...
		if (collect2.stream().findAny().isPresent()) {
			...
			if (averageWGDI.isPresent()) {
				...
				if (isSTOL) {
					...
					// reportWB.getSheet(isheet).setValue(5, row, wgi2.getWgiDI(), numberd0);
					// reportWB.getSheet(isheet).setValue(6, row,
					// wgi2.getWgiDIAfterChangeScenario(), numberd0);
		...
		for (HH hh2 : uniqueHousehold) {
			...
			if (isCopingStrategy) {
				...
				for (ExpandabilityRule expandabilityRule : expandabilityRules) {
					 ...
					 * getExpandabilityCostOfDeficitPurchase(), numberd0);
					 */
					/*
					...
					reportWB.getSheet(isheet).setValue(col, row, prevHHOSolAfterChangeScenario + cashGain, numberd0);
...
//							for (MicroNutrientLevel mn : thisMccwFoodSource.getMicroNutrientLevel()) {
	...
	private void createOIHMNutrientreport(int isheet, Report report, Boolean isnotCopingStrategy) {
		...
		for (HH hh2 : uniqueHousehold) {
			...
			reportWB.getSheet(isheet).setValue(col, row++, hh2.getHhNumber(), numberd0);
		}
		row = 1;
		col = 2;
		printNutrientHeader(isheet, col, row);
		...
		if (model == OIHM) {
			...
			for (HH hh2 : uniqueHousehold) {
				...
				try {
					...
					lssIncome = calcLSS(hh2.getHousehold().getLivestockSales(), type, false);
				...
				reportWB.getSheet(isheet).setValue(col++, row, lsTotal, numberd0);
				reportWB.getSheet(isheet).setValue(col++, row, acslsTotal, numberd0);
				...
				if (isCopingStrategy) {
					...
					for (ExpandabilityRule expandabilityRule : expandabilityRules) {
						...
						reportWB.getSheet(isheet).setValue(col, row, totalIncome, numberd0);
		...
		if (listWealthgroup.stream().filter(p -> p.getWgorder() == wgOrder).findAny().isPresent()) {
			...
			for (WealthGroup wealthGroup2 : listWealthgroup.stream().filter(p -> p.getWgorder() == wgOrder)
				...
				income[6] += calcCropIncome(wealthGroup2.getWealthGroupInterview().get(0).getCrop(), type,
						isAfterChangeScenario, isFoodSubstitution, true);
	...
	public static Double calcWFIncome(Collection<WildFood> wildfoods, String type, Boolean isAfterChangeScenario,
			Boolean isFoodSubstitution, Boolean isOHEA) {
		Double wfTot = 0.0;
		...
		for (WildFood wf : wildfoods) {
			...
			if (isFoodSubstitution && isOHEA) {
				rst = calcRSTforFoodSubstitution(rst, true); /* RST + isFoodSubstitution */
			}
	...
	public static Double calcTransIncome(Collection<Transfer> trans, String type, Boolean isAfterChangeScenario,
		...
		for (Transfer tr : trans) {
			...
			if (type == "cash") {
				...
				rst = tr.getFoodResourceSubType();
				rst = calcRSTforFoodSubstitution(rst, isFoodSubstitution);
				if (isAfterChangeScenario) {
					priceChange = priceYieldVariation(modellingScenario, rst, PRICE);
					/* #528 Transfers only apply Price change */
					// yieldChange = priceYieldVariation(modellingScenario, rst, YIELD);
	...
	public static Double calcEmpIncome(Collection<Employment> emps, String type, Boolean isAfterChangeScenario,
		...
		for (Employment emp : emps) {
			...
			if (type == "cash") {
				...
				if (isFoodSubstitution)
					rst = calcRSTforFoodSubstitution(rst, true);
